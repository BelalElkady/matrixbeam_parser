...
/** \addtogroup l_structures Legacy Data Structures
 ...
 *  Most of the structures are outdated and no longer used for the new BabyLIN API
 *  @{
 */
 ...
 *
 * Use @ref BL_searchBabyLinPorts to retrieve a list of connected BabyLIN-Devices
 *
 * */
typedef struct
{
...
/*!
 *  \param pdest       pointer to BLC_portInfo_t array, where the infos about the recognized Baby-LIN's will be supplied.
 *                     If this pointer is given with NULL, the function will only return the count of available devices.
 *  \param psize       pointer to interger which holds the maximum count of entries, which may be filled by the function into the array pdest.
 *                     If this pointer is given with NULL, the function will only return the count of available devices.
 ...
 *  \return          Status of operation; '=0' means successful, '!=0' otherwise.
 *                   See standard return values for error, or for textual
 *                   representation (for return values < -1000)  getLastDeviceError().
 */
...
/** \addtogroup l_sdf_handling Legacy SDF Information
 ...
 *  If you want to use a pull method to retrieve the data, have a look at the \ref l_pull_handling "Pull Methods" section of the documentation
 *  These functions are outdated and no longer used for the new BabyLIN API.
 *  @{
 */
 ...
 *  \param handle      Handle representing the connection; returned previously by
 *                     BL_open().
 *  \param callback    Pointer to a function call-compatible to  BL_frame_callback_func.
 *  \return            Status of operation; '=0' means successful, '!=0' otherwise.
 *                     See standard return values for error, or for textual
 *                     representation (for return values < -1000)  BL_getLastError().
 */
 ...
 *  \param handle      Handle representing the connection; returned previously by
 *                     BL_open().
 *  \param callback    Pointer to a function call-compatible to BL_signal_callback_func.
 *  \return            Status of operation; '=0' means successful, '!=0' otherwise.
 *                     See standard return values for error, or for textual
 *                     representation (for return values < -1000) BL_getLastError().
 */
 ...
 *  \param handle      Handle representing the connection; returned previously by
 *                     BL_open().
 *  \param callback    Pointer to a function call-compatible to  BL_frame_buserror_func.
 *  \return            Status of operation; '=0' means successful, '!=0' otherwise.
 *                     See standard return values for error, or for textual
 *                     representation (for return values < -1000) BL_getLastError().
 */
int BL_DLLIMPORT  BL_registerBusErrorCallback(BL_HANDLE handle, BL_buserror_callback_func *callback);            // for backwards compatibility
 ...
 *  \param handle      Handle representing the connection; returned previously by
 *                     BL_open().
 *  \param callback    Pointer to a function call-compatible to  BL_debug_callback_func.
 *  \return            Status of operation; '=0' means successful, '!=0' otherwise.
 *                     See standard return values for error, or for textual
 *                     representation (for return values < -1000) BL_getLastError().
 */
 ...
 *  \param handle      Handle representing the connection; returned previously by
 *                     BL_open().
 *  \param callback    Pointer to a function call-compatible to BL_dtl_request_callback_func.
 *  \return            Status of operation; '=0' means successful, '!=0' otherwise.
 *                     See standard return values for error, or for textual
 *                     representation (for return values < -1000) BL_getLastError().
 */
 ...
 *  \param handle      Handle representing the connection; returned previously by
 *                     BL_open().
 *  \param callback    Pointer to a function call-compatible to BL_dtl_response_callback_func.
 *  \return            Status of operation; '=0' means successful, '!=0' otherwise.
 *                     See standard return values for error, or for textual
 *                     representation (for return values < -1000) BL_getLastError().
 */
...
/** \addtogroup l_commands Legacy Command Functions
 ...
 *  \param handle   Handle representing the connection; returned previously by
 *                  BL_open().
 *  \param command  C-string with the command and placeholders (i.e. "setsig %S 1;").
 ...
 *                     representation (for return values < -1000) BL_getLastError().
 */
int BL_DLLIMPORT   BL_sendRawSlaveResponse(BL_HANDLE handle, unsigned char *reqdata, unsigned char *reqmask, unsigned char *data, int length);
 ...
 *  \param handle      Handle representing the connection; returned previously by
 *                     BL_open().
 *  \return            Status of last request operation if >= 0; see BL_DTL_STATUS for
 *                     values. For < 0, see standard return values for error, or for
 *                     textual representation (for return values < -1000) BL_getLastError().
 */
 ...
 *                  representation (for return values < -1000)  BL_getLastError().
 */
int BL_DLLIMPORT  BL_getAnswerTypeByIndex(BL_HANDLE handle, const unsigned int index, BL_ANSWER_TYPE *type, size_t *length);
...
/** \addtogroup l_pull_handling Legacy Pull Methods
 ...
 *  This apporach uses the pull method, i.e. you will not get any information pushed ( see \ref l_callback_handling "Callback Handling" ) when it's received.
 ...
 *  \return           Status of operation; '=0' means successful, '!=0' otherwise.
 *                    See standard return values for error, or for textual
 *                    representation (for return values < -1000)  getLastChannelError().
 */
 ...
 *                     representation (for return values < -1000) BL_getLastError().
 */
int BL_DLLIMPORT  BL_encodeSignal(BL_HANDLE handle, int signalNr, unsigned int value, char *encSignal, size_t *buffLen0, char *encUnit, size_t *buffLen1);
...
/**
  ...
  * @param plinid	   Pointer to int, which gets filled with LIN ID (without parity bits)
  ...
  * @param pframetype  Pointer to int, which gets filled with Lin version of this frame
  */
